...
create policy if not exists "allow read" on public.pollution_daily for select using (true);
...
create policy if not exists "allow read" on public.asgs_sa2_2021 for select using (true);
create policy if not exists "allow read" on public.asgs_sa3_2021 for select using (true);
create policy if not exists "allow read" on public.asgs_sa4_2021 for select using (true);

-- Helper mapping SA2->SA3/SA4 (can be materialized from ABS correspondences)
create view if not exists public.sa2_to_sa3 as
...
create view if not exists public.pollution_daily_sa2 as
...
create view if not exists public.pollution_daily_sa3 as
...
create view if not exists public.pollution_daily_sa4 as
